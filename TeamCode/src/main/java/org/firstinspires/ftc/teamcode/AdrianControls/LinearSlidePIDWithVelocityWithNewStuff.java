...
public class LinearSlidePIDWithVelocityWithNewStuff {
    ...
    public static final double LIFT_MASS_FOR_MANUAL = 0.5;
    ...
    private PIDFController controllerForManual;
    public boolean isMotionProfillingBeingUsed = true;
    private boolean suppressMotionProfiling = false;
    public LinearSlidePIDWithVelocityWithNewStuff(HardwareMap hardwareMap) {
        Realizer.realize(this, hardwareMap);

        controllerForManual = new PIDFController(new PIDCoefficients(10.0, 0, 0),//1.0,0.0,0.08
                MOTOR_CONSTANTS.kB / (diameterOfSpool/2),
                (MOTOR_ANGULAR_MOMENT_OF_INERTIA + LIFT_MASS_FOR_MANUAL*Math.pow((diameterOfSpool/2),2)) / (diameterOfSpool/2),
                MOTOR_CONSTANTS.kStatic,
                (x,dx) -> LIFT_MASS_FOR_MANUAL * 9.8066  * (diameterOfSpool/2)
    ...
    public void setTargetPosition(double height) {
        ...
        if(height<=maxHeight || height>=minHeight) {
            controllerForManual.setTargetPosition(height);
        }
    ...
    public void update() {
        if(isMotionProfillingBeingUsed) {
            ...
            //controllerForManual.update(currentPosition, currentVelocity);
        ...
        else
        {
            double currentPosition = getCurrentPosition();
            double currentVelocity = getCurrentVelocity();
            double powerToApply = controllerForManual.update(currentPosition, currentVelocity);
            ...
            if (Math.abs(getCurrentPosition() - controllerForManual.getTargetPosition()) <= 0.03) {
                LinearSlideMode = LinearSlideStates.Idle;
            }
            if(!liftMagneticSwitch.getState() && controllerForManual.getTargetPosition()==0){
                powerToApply=0;
            }
