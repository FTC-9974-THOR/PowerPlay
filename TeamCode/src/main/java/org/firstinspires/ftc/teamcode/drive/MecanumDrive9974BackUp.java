 ...
 */
@Config
public class MecanumDrive9974BackUp extends MecanumDrive {
   ...
   public static PIDFCoefficients SHOOTER_PID = new PIDFCoefficients(35, 0, 7, 14);
    ...
    public Servo leftWheel, rightWheel;
    ...
    public int targetEncoderCountLevel0 = 921;//1200;
    public int targetEncoderCountLevel1 = 921;//1200;
    public int targetEncoderCountLevel2 = 1605;//2200;
    public int targetEncoderCountLevel3 = 2390;//3200;
    public int targetEncoderCountLevel4 = 2458;//3300;
    ...
    public MecanumDrive9974BackUp(HardwareMap hardwareMap) {        super(kV, kA, kStatic, TRACK_WIDTH, TRACK_WIDTH, LATERAL_MULTIPLIER);
      ...
      //  arm = hardwareMap.get(DcMotorEx.class, "frontEncoder");
       ...
       shooter = hardwareMap.get(DcMotorEx.class, "SlideMotor");
...
//        digitalTouch.setMode(DigitalChannel.Mode.INPUT);
   ...
   //     leftServo = hardwareMap.get(Servo.class, "leftServo");
    ...
    public boolean IsArmLifterBusy(){
        return ArmLifterMode != ArmLifterState.Idle;
    }
    ...
    public void armFirstLevel () {
        int startingPosition = SlideMotor.getCurrentPosition();
        SlideMotor.setPower(0.3);
        SlideMotor.setTargetPosition(176);
   ...
   public void shootPowerShots () {
    ...
    public void ArmLifter (int level,int pidTimerInSeconds){
        ...
        int initialPosition = SlideMotor.getCurrentPosition();
        timerForPid.reset();
        double powerToApply = 0.0;
        /*
        if(SlideMotor.getCurrentPosition()-targetEnconderCountLevel<0){
         ...
         */
        if(SlideMotor.getCurrentPosition() - targetEnconderCountLevel < -100){
            powerToApply = -0.7;
        }
        else if(SlideMotor.getCurrentPosition() - targetEnconderCountLevel > 100){
            powerToApply = 0.7;
        }
        else{
            powerToApply = 0.0;
            SlideMotor.setPower(powerToApply);
        }
        /*
...
*/
        while(Math.abs(SlideMotor.getCurrentPosition() - targetEnconderCountLevel)> 100 && timerForPid.seconds()<pidTimerInSeconds)
        {
            /*
            powerToApply = Math.abs(SlideMotor.getCurrentPosition() - targetEnconderCountLevel) * 1.0/Math.abs(initialPosition-targetEnconderCountLevel);
            ...
            Log.d("CurrentPosition ", String.valueOf(SlideMotor.getCurrentPosition()));
  ...
  /*      if(targetEnconderCountLevel==0){
        ...
        {
            SlideMotor.setPower(armMinPowerDuringHold);
        }
   ...
   */
    ...
    public ArmLifterState ArmLifterMode;

    public void ArmLifterAsyncUpdate(int level)
    {
        ArmLifterMode = ArmLifterState.Idle;
        int targetEnconderCountLevel=targetEncoderCountLevel3;
        if(level ==  4){
            targetEnconderCountLevel = targetEncoderCountLevel4;
        }
        ...
        if(SlideMotor.getCurrentPosition() - targetEnconderCountLevel < -100){
            powerToApply = -0.8;
            ArmLifterMode = ArmLifterState.Moving;
        }
        else if(SlideMotor.getCurrentPosition() - targetEnconderCountLevel > 100){
            powerToApply = 0.7;
            ArmLifterMode = ArmLifterState.Moving;
        ...
        Log.d("CurrentPosition ", String.valueOf(SlideMotor.getCurrentPosition()));
        Log.d("Targe Post", String.valueOf(targetEnconderCountLevel));
        Log.d("State", String.valueOf(ArmLifterMode));
    ...
    public double ArmLifterPowerToApplyCalculation(int level,int pidTimerInSeconds){
            ...
            int initialPosition = SlideMotor.getCurrentPosition();
            double powerToApply = 0.0;
            if(SlideMotor.getCurrentPosition() > targetEnconderCountLevel+500 || SlideMotor.getCurrentPosition() > targetEnconderCountLevel-500 ){
              //  SlideMotor.setDirection(DcMotorSimple.Direction.FORWARD);
                powerToApply=-1.0;
                Log.d("Came Here: ", "Came here: ");
            }
